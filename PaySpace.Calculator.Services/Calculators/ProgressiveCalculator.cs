...
namespace PaySpace.Calculator.Services.Calculators
{
    public class ProgressiveCalculator : IProgressiveCalculator
    {
        ...
        public ProgressiveCalculator(CalculatorContext calculatorContext)
        {
            _dbContext = calculatorContext;
        }
        public async Task<CalculateResult> CalculateAsync(decimal income)
        {
            var settings = await _dbContext.CalculatorSettings.Where(x => x.Calculator == CalculatorType.Progressive).ToListAsync();
